...
###
    ...
    def motion_detect(counter, move_choice, move_prob_list, move_movements):
        sim_move = random.randint(0,1)
        ###
        #Next bit to detect movement. each movement can be 2 seconds and needs to have  
        ### if (GPIO.input(setup.movdetect_pin) == 0):
        if sim_move == 0:
        ...
        ### else (GPIO.input(setup.movdetect_pin) == 1):
...
def run_laser():
    ...
    try:
        while True:
            ...
            counter = 1
            ###
            movement_gen.motion_detect(counter, move_choice, move.prob_list, move.movements)
